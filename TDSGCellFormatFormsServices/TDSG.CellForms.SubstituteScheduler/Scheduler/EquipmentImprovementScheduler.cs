...
{
    public  class EquipmentImprovementScheduler
    {
        ...
        public EquipmentImprovementScheduler(IConfiguration configuration, TdsgCellFormatDivisionContext cellContext, AepplNewCloneStageContext dbContext)
        {
        ...
        public void EquipmentSubstituteCheck()
        {
            try
            {
                ...
                if (substitutes.Count > 0)
                {
                    foreach (var item in substitutes)
                    {
                        ...
                        var approverTasks = _cellContext.EquipmentImprovementApproverTaskMasters
                       .Where(x =>
                           (
                           ...
                           && (x.FormType == ProjectType.Equipment.ToString())
                       // && x.IsSubstitute
                       ).ToList();
            ...
            catch (Exception ex)
            {
                ...
                commonHelper.LogException(ex, "Equipment Substitute Scheduler");
                //return false;
