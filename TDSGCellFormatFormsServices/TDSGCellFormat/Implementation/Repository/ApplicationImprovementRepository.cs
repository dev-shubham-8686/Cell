    ...
    public class ApplicationImprovementRepository : BaseRepository<EquipmentImprovementApplication>, IApplicationImprovementRepository
    {
        ...
        public async Task<AjaxResult> InsertDelegate(DelegateUser request)
        {
            ...
            catch (Exception ex)
            {
                ...
                commonHelper.LogException(ex, "InsertDelegate");
        ...
        public EquipmentImprovementApplicationAdd GetById(int Id)
        {
            ...
            var seqNotwo = _context.EquipmentImprovementApproverTaskMasters.Where(x => x.EquipmentImprovementId == Id &&
                                                       x.Status == ApprovalTaskStatus.UnderAmendment.ToString() && x.WorkFlowlevel == 2 && x.IsActive == true).Select(x => x.SequenceNo).FirstOrDefault();
            ...
            if (equipmentPcrn != null)
            {
                ...
                {
                    PcrnAttachmentId = attach.PCRNId,
                    PcrnDocName = attach.PCRNDocFileName,
                    PcrnFilePath = attach.PCRNDocFilePath
                }).ToList();
            }
                ...
                else
                {
                    ...
                    if (report.ResultAfterImplementation != null)
                    {
                        ...
                        else if (existingReport.IsLogicalAmend == true)
                        {
                            ...
                            if (data.IsResultAmendSubmit == true)
                            {
                                var nextData = await EditResult(report);
                                if (nextData.StatusCode == Enums.Status.Success)
                                {
        ...
        public async Task<AjaxResult> SubmitRequest(int equipmentId, int? createdBy)
        {
            ...
            catch (Exception ex)
            {
                ...
                res.StatusCode = Enums.Status.Error;
                var commonHelper = new CommonHelper(_context, _cloneContext);
                commonHelper.LogException(ex, "Equipment SubmitRequest");
                //return res;
            }
            return res;
        }
        ...
        public async Task<AjaxResult> Resubmit(int equipmentId, int? createdBy)
        {
            ...
            try
            {
                ...
                var equipment = _context.EquipmentImprovementApplication.Where(x => x.EquipmentImprovementId == equipmentId && x.IsDeleted == false).FirstOrDefault();
                if (equipmentApproverTask.SequenceNo == 6 && equipment.ToshibaApprovalRequired == true)
                {
                    equipment.ToshibaApprovalRequired = null;
                    equipment.ToshibaApprovalComment = null;
                    equipment.ToshibaApprovalDate = null;
                    equipment.ToshibaApprovalTargetDate = null;
                    equipment.ToshibaApprovedRemarks = null;
                    equipment.IsPcrnRequired = null;
                    await _context.SaveChangesAsync();
                }
                if(equipmentApproverTask.SequenceNo == 2 && equipment.ToshibaTeamDiscussion == true)
                {
                   equipment.ToshibaTeamDiscussion = null;
                   equipment.ToshibaDiscussionTargetDate = null;
                   equipment.ToshibaDicussionComment = null;
                   equipment.WorkFlowLevel = 1;
                    await _context.SaveChangesAsync();
                }
                equipmentApproverTask.Status = ApprovalTaskStatus.InReview.ToString();
        ...
        public async Task<AjaxResult> EditFormData(EquipmentImprovementApplicationAdd report)
        {
            ...
            try
            {
                ...
                else if (report.IsSubmit == true && report.IsAmendReSubmitTask == true && existingReport.IsLogicalAmend == true)
                {
                    ...
                    var notificationHelper = new NotificationHelper(_context, _cloneContext);
                    await notificationHelper.SendEquipmentEmail(existingReport.EquipmentImprovementId, EmailNotificationAction.LogicalResubmit, string.Empty, 0);
                    if (adminId)
                    {
        ...
        public async Task<AjaxResult> EditResult(EquipmentImprovementApplicationAdd report)
        {
            ...
            try
            {
                ...
                {
                    existingReport.Status = ApprovalTaskStatus.UnderAmendment.ToString();
                }
                else
                {
                    ...
                    if (data.TargetDate != null && data.ActualDate != null && !adminId)
                    {
                        ...
                        if (data.ResultStatus == string.Empty || data.ResultStatus == null)
                        {
                ...
                var existingPcrnAttachment = _context.EquipmentPCRNAttachments.Where(x => x.EquipmentImprovementId == existingReport.EquipmentImprovementId).ToList();
                MarkAsDeleted(existingPcrnAttachment, existingReport.CreatedBy, DateTime.Now);
                _context.SaveChanges();

                if (data.PcrnAttachments != null)
                {
                    foreach (var attach in data.PcrnAttachments)
                    {
                        //var updatedUrl = attach.PcrnFilePath.Replace($"/{report.CreatedBy}/", $"/{existingReport.EquipmentImprovementNo}/");
                        var existingAttachData = _context.EquipmentPCRNAttachments.Where(x => x.EquipmentImprovementId == report.EquipmentImprovementId && x.PCRNId == attach.PcrnAttachmentId).FirstOrDefault();
                        if (existingAttachData != null)
                        {
                            existingAttachData.PCRNDocFileName = attach.PcrnDocName;
                            existingAttachData.PCRNDocFilePath = attach.PcrnFilePath;
                            existingAttachData.IsDeleted = false;
                            existingAttachData.ModifiedBy = attach.ModifiedBy;
                            existingAttachData.ModifiedDate = DateTime.Now;
                        }
                        else
                        {
                            ...
                            var attachment = new EquipmentPCRNAttachment()
                            {
                                EquipmentImprovementId = existingReport.EquipmentImprovementId,
                                PCRNDocFileName = attach.PcrnDocName,
                                PCRNDocFilePath = attach.PcrnFilePath,
                                IsDeleted = false,
                                CreatedBy = attach.CreatedBy,
                                CreatedDate = DateTime.Now,
                            };
                            _context.EquipmentPCRNAttachments.Add(attachment);
                        }
                        await _context.SaveChangesAsync();
                    }
                }
        ...
        public async Task<AjaxResult> ResultResubmit(int equipmentId, int? userId)
        {
            ...
            try
            {
                ...
                if(equipment != null && equipmentApproverTask != null)
                {
                ...
                if (userId == adminId)
                {
                    InsertHistoryData(equipmentId, FormType.EquipmentImprovement.ToString(), "Admin", "ReSubmit the Form", equipment.Status, Convert.ToInt32(adminId), HistoryAction.ReSubmitted.ToString(), 0);
                }
                else
                {
                    InsertHistoryData(equipmentId, FormType.EquipmentImprovement.ToString(), "Requestor", "ReSubmit the Form", equipment.Status, Convert.ToInt32(userId), HistoryAction.ReSubmitted.ToString(), 0);
                }
            ...
            try
            {
                ...
                if (equipmentTask != null)
                {
                    if (equipmentTask.WorkFlowLevel == 1)
                    {
                        ...
                        equipmentTask.ToshibaTeamDiscussion = null;
                        equipmentTask.ToshibaApprovalRequired = null;
                        equipmentTask.ToshibaApprovalComment = null;
                        equipmentTask.ToshibaApprovalDate = null;
                        equipmentTask.ToshibaApprovalTargetDate = null;
                        equipmentTask.ToshibaApprovedRemarks = null;
                        equipmentTask.ToshibaDiscussionTargetDate = null;
                        equipmentTask.ToshibaDicussionComment = null;
                        equipmentTask.ToshibaApprovedRemarks = null;
                    ...
                    else
                    {
                        ...
                        equipmentTask.ModifiedBy = data.userId;
                        equipmentTask.ModifiedDate = DateTime.Now;
                        equipmentTask.IsLogicalAmend = false;
                        // mention the WorkFlow status 
                        await _context.SaveChangesAsync();
                ...
                if (data.Type == ApprovalStatus.Approved)
                {
                    ...
                    if (data.EquipmentApprovalData != null)
                    {
                        ...
                        else if (approvalData.IsToshibaDiscussion == false)
                        {
                            ...
                            if (approvalData.IsPcrnRequired == false)
                            {
                                equipment.IsPcrnRequired = approvalData.IsPcrnRequired;
                    ...
                    if (currentApproverTask != null)
                    {
                        ...
                        if (nextTask != null)
                        {
                            ...
                            if (currentAssignedUser == nextAssignedUser
                                && (nextTask.WorkFlowlevel != 1 || (nextTask.SequenceNo != 2 && nextTask.SequenceNo != 6)))
                            {
                                ...
                                if (nextPendingTask != null)
                                {
                                    ...
                                    if (nextPendingTask.WorkFlowlevel == 1)
                                    {
                                    ...
                                    else if (nextPendingTask.WorkFlowlevel == 2)
                                    {
        ...
        public async Task<AjaxResult> UpdateTargetDates(EquipmentApprovalData data)
        {
            ...
            {
                var equipment = _context.EquipmentImprovementApplication.Where(x => x.EquipmentImprovementId == data.EquipmentId && x.IsDeleted == false).FirstOrDefault();
                var equipmentToshibaDis = _context.EquipmentImprovementApproverTaskMasters.Where(x => x.EquipmentImprovementId == data.EquipmentId && x.IsActive == true && x.WorkFlowlevel == 1 && x.SequenceNo == 2).FirstOrDefault();
                var equipmentToshibaApp = _context.EquipmentImprovementApproverTaskMasters.Where(x => x.EquipmentImprovementId == data.EquipmentId && x.IsActive == true && x.WorkFlowlevel == 1 && x.SequenceNo == 6).FirstOrDefault();
                var adminId = _context.AdminApprovers.Where(x => x.FormName == ProjectType.Equipment.ToString() && x.IsActive == true).Select(x => x.AdminId).FirstOrDefault();
        ...
        public async Task<(List<EquipmentApproverTaskMasterAdd> WorkflowOne, List<EquipmentApproverTaskMasterAdd> WorkflowTwo)> GetEquipmentWorkFlowData(int equipmentId)
        {
            ...
            // Fetch the equipment details for validation
            var equipment = await _context.EquipmentImprovementApplication.FindAsync(equipmentId);
            ...
            foreach (var entry in approverData)
            {
                ...
                {
                    // Check if SequenceNo is 6 and ToshibaApprovalRequired needs validation
                    if (entry.SequenceNo == 6 && equipment != null && equipment.ToshibaApprovalRequired == true &&
                        equipment.ToshibaApprovedRemarks != null)
                    {
                        entry.Comments = equipment.ToshibaApprovedRemarks;
                    }
        ...
        public ApproverTaskId_dto GetCurrentApproverTask(int equipmentId, int userId)
        {
           ...
           || x.Status == ApprovalTaskStatus.ToshibaTechnicalReview.ToString() || x.Status == ApprovalTaskStatus.LogicalAmendmentInReview.ToString()) && x.IsActive == true);
            ...
            if (materialDelegateApprovers != null)
            {
                ...
                data.userId = materialDelegateApprovers.DelegateUserId ?? 0;
        ...
        private static readonly Dictionary<string, string> ColumnHeaderMapping = new Dictionary<string, string>
{
        ...
        public async Task<AjaxResult> ExportToPdf(int equipmentId)
        {
            ...
            try
            {
                ...
                string QcManagerName = approvalData.FirstOrDefault(a => a.SequenceNo == 6 && a.ActionTakenBy != null && (a.Status == ApprovalTaskStatus.Approved.ToString() || a.Status == ApprovalTaskStatus.Rejected.ToString()))?.employeeNameWithoutCode ?? "N/A";
                string QcManagerComments = approvalData.FirstOrDefault(a => a.SequenceNo == 6 && a.ActionTakenBy != null && (a.Status == ApprovalTaskStatus.Approved.ToString() || a.Status == ApprovalTaskStatus.Rejected.ToString()))?.Comments ?? string.Empty;
                string QcManagerdate = approvalData.FirstOrDefault(a => a.SequenceNo == 6 && a.ActionTakenBy != null && (a.Status == ApprovalTaskStatus.Approved.ToString() || a.Status == ApprovalTaskStatus.Rejected.ToString()))?.ActionTakenDate?.ToString("dd-MM-yyyy") ?? string.Empty;

                string? approvedByAdvisor = approvalData.FirstOrDefault(a => a.SequenceNo == 2 && a.ActionTakenBy != null && (a.Status == ApprovalTaskStatus.Approved.ToString() || a.Status == ApprovalTaskStatus.Rejected.ToString()))?.employeeNameWithoutCode;
                ...
                sb.Replace("#Advisor#", approvedByAdvisor ?? "N/A");
                ...
                sb.Replace("#AdvisorName#", approvedByAdvisor ?? string.Empty);
                ...
                sb.Replace("#ResultStatus#", equipmentData?.ResultStatus ?? string.Empty);
                ...
                if((equipmentData?.ToshibaApprovalRequired == false || equipmentData?.ToshibaApprovalRequired == null) && (equipmentData?.Status == ApprovalTaskStatus.Approved.ToString() || equipmentData?.Status == ApprovalTaskStatus.Completed.ToString() || equipmentData?.Status == ApprovalTaskStatus.LogicalAmendmentInReview.ToString()))
                {
                    sb.Replace("#ToshibaApprovalRequiredChecked#", "");
                    sb.Replace("#NoToshibaApprovalRequiredChecked#", "checked");
                }

                var equipmentApproval = _context.EquipmentImprovementApproverTaskMasters.Where(x => x.WorkFlowlevel == 1 && x.EquipmentImprovementId == equipmentId && x.IsActive == true && x.SequenceNo == 6 && x.Status == ApprovalStatus.Approved.ToString()).FirstOrDefault();
                var equipmentRejection = _context.EquipmentImprovementApproverTaskMasters.Where(x => x.WorkFlowlevel == 1 && x.EquipmentImprovementId == equipmentId && x.IsActive == true && x.SequenceNo == 6 && x.Status == ApprovalStatus.Rejected.ToString()).FirstOrDefault();

                if (equipmentData?.ToshibaApprovalRequired == true && equipmentApproval != null)
                {
                    sb.Replace("#ToshibaApproved#", "checked");
                    sb.Replace("#ToshibaNotApproved#", "");
                }
                if (equipmentData?.ToshibaApprovalRequired == true && equipmentRejection != null)
                {
                    sb.Replace("#ToshibaApproved#", "");
                ...
                else
                {
                    ...
                    sb.Replace("#JudgeBy#", string.Empty);
                ...
                converter.Options.MarginTop = 12;
