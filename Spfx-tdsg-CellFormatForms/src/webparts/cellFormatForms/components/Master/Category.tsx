...
const CategoryType = () => {
  const [data, setData] = useState<CategoryType[]>([
    ...
    },
    {
      id: 5,
      SrNo: "05",
      CategoryType: "Category5",
    },
  ]);

  const [editModalVisible, setEditModalVisible] = useState<boolean>(false);
  const [viewModalVisible, setViewModalVisible] = useState<boolean>(false);
  const [selectedRecord, setSelectedRecord] = useState<CategoryType | null>(null);
  ...
  const EditHandler = (record: CategoryType): void => {
  ...
  const DeleteHandler = (record: CategoryType): void => {
  ...
  const ViewHandler = (record: CategoryType): void => {
    setSelectedRecord(record);
  ...
  const columns: ColumnsType<CategoryType> = [
    ...
    },
    {
      ...
      ...getColumnSearchProps("CategoryType"), // Apply styling function
    },
    {
  ...
  const [tableData, setTableData] = useState<CategoryType[]>(data);

  const SearchTableHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    const searchText = e.target.value;
    setSearchTableText(searchText);
    const filteredData = data.filter(item => item.CategoryType.toLowerCase().includes(searchText.toLowerCase()));
      ...
      >
        {selectedRecord && (
          <div>
            <input
              type="text"
              value={selectedRecord.CategoryType}
              onChange={(e) =>
                setSelectedRecord({ ...selectedRecord, CategoryType: e.target.value })
              }
              style={{ width: "100%" }}
            />
          </div>
