...
type DataIndex = keyof TroubleType;
...
const TroubleType = () => {
  const [data, setData] = useState<TroubleType[]>([
    {
      id: 1,
      SrNo: "01",
      TroubleType: "Machine break down",
    },
    {
      id: 2,
      SrNo: "02",
      TroubleType: "Machine works improperly",
    },
    {
      id: 3,
      SrNo: "03",
      TroubleType: "Wrong machine setting found",
    },
    {
      id: 4,
      SrNo: "04",
      TroubleType: "Abnormal NG",
    },
    {
      id: 5,
      SrNo: "05",
      TroubleType: "Too much NG",
    },
    {
      id: 6,
      SrNo: "06", 
      TroubleType: "NG found accidentally",
    },
  ]);

  const [editModalVisible, setEditModalVisible] = useState<boolean>(false);
  const [viewModalVisible, setViewModalVisible] = useState<boolean>(false);
  const [selectedRecord, setSelectedRecord] = useState<TroubleType | null>(null);
  ...
  const EditHandler = (record: TroubleType): void => {
  ...
  const DeleteHandler = (record: TroubleType): void => {
  ...
  const ViewHandler = (record: TroubleType): void => {
    setSelectedRecord(record);
  ...
  const columns: ColumnsType<TroubleType> = [
    {
      title: "Sr. No.",
      dataIndex: "SrNo",
      key: "SrNo",
    ...
    {
      title: "TroubleType Type",
      dataIndex: "TroubleType",
      key: "TroubleType",
      render: (text) => <p className="text-cell">{text}</p>,
      width: "50%",
      sorter: (a, b) => a.TroubleType.localeCompare(b.TroubleType),
      ...getColumnSearchProps("TroubleType"), // Apply styling function
    },
    {
  ...
  const [tableData, setTableData] = useState<TroubleType[]>(data);

  const SearchTableHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    const searchText = e.target.value;
    setSearchTableText(searchText);
    const filteredData = data.filter(item => item.TroubleType.toLowerCase().includes(searchText.toLowerCase()));
      ...
      >
        {selectedRecord && (
          <div>
            <input
              type="text"
              value={selectedRecord.TroubleType}
              onChange={(e) =>
                setSelectedRecord({ ...selectedRecord, TroubleType: e.target.value })
              }
              style={{ width: "100%" }}
            />
          </div>
